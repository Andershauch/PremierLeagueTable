...
## 2.4.0 - 2026-08-19
- Fixed the focus team never being highlighted in the WSL table. The saved focus club resolves to a competition-specific name (`Tottenham Women`), while the WSL feed has served PL-style names (`Tottenham Hotspur`) since the WPLL source became primary in 2.2.0 — neither string contains the other, so the highlight stopped matching. `PLT_Standings_Service::matches_focus_team()` now compares canonical club identity via `PLT_Club_Map` instead of comparing two provider-specific display names, with the old string matching kept as a fallback for hand-typed `focus_team` shortcode values. The failure was partial and therefore easy to miss: clubs whose women's name appends a suffix (`Arsenal` → `Arsenal WFC`) kept matching by substring luck, while clubs whose name replaces it (Tottenham, West Ham) did not.
- Fixed the Legacy preset discarding the table font, font scale, and density. Those three controls live in the "Preset and layout" group, which is deliberately not `customOnly`, so they are visible and saveable while Legacy is selected — but `get_frontend_theme_config()` returned an empty style attribute and hardcoded `plt-font-medium plt-density-comfortable` for Legacy, silently throwing all three away. Legacy now emits them while still omitting the custom-only colour and per-element typography tokens, so the released Spurs look is unchanged.
- Bundled the Archivo typeface (SIL Open Font License 1.1, `assets/fonts/`) and declared it via `@font-face` in both frontend stylesheets. "Apex New" is Tottenham's proprietary face and cannot be redistributed; the plugin previously just named it in a CSS stack, so every visitor without it installed silently got a generic sans-serif. The stack is now `"Apex New", "Archivo", Arial, …`, so anyone with the real font still sees it and everyone else gets a deliberate grotesque. Archivo is also selectable on its own. Served from the plugin folder, so there is no third-party request per page view.
- The `@font-face` is declared in `next-match.css` as well as `frontend.css`, because the two stylesheets are enqueued independently and a page carrying only `[pl_next_match]` never loads the table stylesheet.
- Fixed the admin live preview showing Legacy with a font and density the frontend would not actually apply.
- Declared webfont extensions as binary in `.gitattributes`, so end-of-line normalisation cannot corrupt a bundled font file.
